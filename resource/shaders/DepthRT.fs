...
void main(){
    ...
    gl_FragColor = depth;//vec4(vec3(gl_FragCoord.z), 1.0); //vec4(-Position_viewspace.z); //vec4(vec3(gl_FragCoord.z), 1.0); //depth; //Position_screenspace; //gl_FragCoord.z / gl_FragCoord.w;
}