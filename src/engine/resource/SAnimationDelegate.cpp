//////////////////////////////////////////////////////
// file: SAnimationDelegate.cpp 
// created by zhangxiang on 09-01-10
// define of the class SAnimationDelegate
// SAnimationDelegate is a class ...
//////////////////////////////////////////////////////

// INCLUDES //////////////////////////////////////////
#include "SAnimationDelegate.h"
...
namespace Sagitta{
	...
	void SAnimationDelegate::loadImpl(const StdString &aFileName, SResource *outResource){
		THROW_SAGI_EXCEPT(SException::ERR_NOT_IMPLEMENTED,
			"Normal Animation resource loading is not implemented",
			"SAnimationDelegate::loadImpl");
	}
	...
	void SMotionClipDelegate::loadImpl(const StdString &aFileName, SResource *outResource){
		// test file type
		...
		if(lastdot != StdString::npos){
			...
			if(postfix == ".bvh"){
				// .obj file
				loadMotionClip_bvh(aFileName, static_cast<SMotionClip*>(outResource));
			}else{
				THROW_SAGI_EXCEPT(SException::ERR_NOT_IMPLEMENTED,
					postfix + " is not a supported file type.",
					"SMotionClipDelegate::loadImpl");
			}
		}else{
			THROW_SAGI_EXCEPT(SException::ERR_NOT_IMPLEMENTED,
	...
	//  [1/11/2009 zhangxiang]
	void SMotionClipDelegate::loadMotionClip_bvh(const StdString &aFileName, SMotionClip *outMotionClip){
		SFile file(aFileName);
		if(!file){
			THROW_SAGI_EXCEPT(SException::ERR_INTERNAL_ERROR,
				"Cannot open file \"" + aFileName + "\".",
				"SMotionClipDelegate::loadMotionClip_bvh");
		}
		...
		std::vector<SAnimaJoint*> joints(jointnum);
