...
namespace Sagitta{
	...
	*/
	class _SG_KernelExport sgCamera : public sgEsse{
	// enum defines
	public:
	...
	public:
		sgCamera(void);
		sgCamera(const Vector3 &aUpDir, const Vector3 &aShootDirOrTargetPos, const CameraMode &aCM,
				Real aFovY = Math::PI_DIV_3, Real aNear = 1.0f, Real aFar = 10000.0f);
		sgCamera(const StdString &asName, const Vector3 &aUpDir, const Vector3 &aShootDirOrTargetPos, const CameraMode &aCM,
				Real aFovY = Math::PI_DIV_3, Real aNear = 1.0f, Real aFar = 10000.0f);
		~sgCamera(void);
	...
	public:
		...
		sgSceneObject *clone(void) const;
		...
		*/
		virtual void translate(const Vector3 &aTranslate, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);
		...
		/** Moves the node along arbitrary axes.
			...
			@param
				relativeTo The space which this transform is relative to.
		*/
		void translate(const Matrix3 aAxes, const Vector3 aTranslate, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);

		/** Rotate the node around the Z-axis. */
		void roll(const Radian &aAngle, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);
		void rotationZ(const Radian &aAngle, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);

		/** Rotate the node around the X-axis. */
		void pitch(const Radian &aAngle, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);
		void rotationX(const Radian &aAngle, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);

		/** Rotate the node around the Y-axis. */
		void yaw(const Radian &aAngle, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);
		void rotationY(const Radian &aAngle, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);

		/** Rotate the node around an arbitrary axis. */
		void rotate(const Vector3 &aAxis, const Radian& aAngle, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);

		/** Rotate the node around an arbitrary axis using a Quaternion. */
		void rotate(const Quaternion &aq, sgNode::TransformSpace aRelativeTo = sgNode::TS_LOCAL);
