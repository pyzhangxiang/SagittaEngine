//////////////////////////////////////////////////////
// file: SEntity.cpp @ 2008-1-31 by Zhang Xiang
// defines of the class SEntity
// SEntity is a class ...
//////////////////////////////////////////////////////
...
namespace Sagitta{
	...
	uLong SEntity::ms_iEntityCount = 1;

	//  [1/4/2009 zhangxiang]
	SEntity::SEntity(void)
	: SRenderable(), m_bGeometryPrepared(false),
	...
	m_pIndexData(0), m_pFaceNormalBuffer(0)*/{
		StdString name = "Entity" + SString::to_string(SEntity::ms_iEntityCount++);
		SSceneObject::_setName(name);
	}

	//  [1/4/2009 zhangxiang]
	SEntity::SEntity(uInt aPolyType,
					size_t aVertexNum,
					size_t aPolyNum,
					bool aSmooth /* = true */,
	...
	m_bCounterClockWise(aCounterClockWise)/*, m_pFaceNormalBuffer(0)*/{
		...
		m_pVertexData = new SVertexData(m_iVertexNum);
		m_pIndexData = new SIndexData(m_iPolyType, m_iPolyNum);
	}

	//  [1/4/2009 zhangxiang]
	SEntity::SEntity(const StdString& aName, 
					uInt aPolyType,
					size_t aVertexNum,
					size_t aPolyNum,
					bool aSmooth /* = true */,
					bool aCounterClockWise/* = true*/)
	: SRenderable(aName), m_bGeometryPrepared(false),
	m_iPolyType(aPolyType), m_iPolyNum(aPolyNum),
	m_iVertexNum(aVertexNum), m_bSmooth(aSmooth), m_bNormalOuter(true),
	m_bCounterClockWise(aCounterClockWise)/*, m_pFaceNormalBuffer(0)*/{
		if(m_iPolyType < 0 || m_iPolyNum < 0 || m_iVertexNum < 0){
	...
	SSceneObject *SEntity::clone() const{
		SEntity *ret = new SEntity;
		// construct code, for future ...
		return ret;
	}

	bool SEntity::geometryPrepared(void) const{
		return m_bGeometryPrepared;
	}

	//  [1/6/2009 zhangxiang]
	void SEntity::prepareGeometry(void){
	...
	//  [1/4/2009 zhangxiang]
	uInt SEntity::polyType(void) const{
		return m_iPolyType;
	}

	//  [1/4/2009 zhangxiang]
	size_t SEntity::polyNum() const{
		return m_iPolyNum;
	}

	//  [1/4/2009 zhangxiang]
	bool SEntity::isSmooth(void) const{
		return m_bSmooth;
	}

	void SEntity::setSmooth(bool aSmooth){
		m_bSmooth = aSmooth;
	}

	bool SEntity::normalOuter(void) const{
		return m_bNormalOuter;
	}

	void SEntity::setNormalOuter(bool aNormalOuter){
		m_bNormalOuter = aNormalOuter;
	}

	//  [1/5/2009 zhangxiang]
	void SEntity::setupNormals(void){
		if(!m_pVertexData || !m_pIndexData){
			THROW_SAGI_EXCEPT(SException::ERR_INVALID_STATE,
							"Invalid entity without vertex data.",
							"SEntity::setupNormals");
		}

		SVertexBufferElement *vBuffer = m_pVertexData->getElement(SVertexBufferElement::ET_VERTEX);
		SVertexIndexBuffer *vIndex = m_pIndexData->getElement(SVertexBufferElement::ET_VERTEX);
		if(!vBuffer || !vIndex){
			THROW_SAGI_EXCEPT(SException::ERR_INVALID_STATE,
							"Invalid entity without vertex data.",
							"SEntity::setupNormals");
		...
		SVertexIndexBuffer *vnIndex = m_pIndexData->getElement(SVertexBufferElement::ET_NORMAL);
		if(vnIndex){
			vnIndex->resize(0);
		}
		...
		if(!m_pFaceNormalBuffer){
			m_pFaceNormalBuffer = new SBuffer(m_iPolyNum * sizeof(Vector3));
		}else{
		...
		SVertexBufferElement *vnBuffer = m_pVertexData->getElement(SVertexBufferElement::ET_NORMAL);
		if(m_bSmooth){
			...
			if(!vnBuffer){
				...
				vnBuffer = m_pVertexData->createElement(SVertexBufferElement::ET_NORMAL,
					3, normalNum);
			}else{
	...
	//  [1/5/2009 zhangxiang]
	void SEntity::setupNormals(bool aNormalOuter){
		m_bNormalOuter = aNormalOuter;
		setupNormals();
	} //#### end computeNormal

	/** for future ...
	int SEntity::computeEdgeNormal(void) const{
	...
	*/
	...
	void SEntity::trianglate(void){
		...
		SIndexData::ConstIterator it = m_pIndexData->getConstIterator();
		for(int i=0; it.hasMoreElements(); ++it, ++i){
			SVertexIndexBuffer *indexBuffer = it.value();
...
//					newFace = i * prelastVertexInOnePoly + j;
	...
	//  [1/5/2009 zhangxiang]
	void SEntity::calCenterAndRadius(void){
		if(!m_pVertexData || !m_pIndexData){
			THROW_SAGI_EXCEPT(SException::ERR_INVALID_STATE,
				"Invalid entity without vertex data.",
				"SEntity::calCenterAndRadius");
		}
	...
	//  [1/6/2009 zhangxiang]
	void SEntity::locateToCenter(void){
		if(!m_pVertexData || !m_pIndexData){
			THROW_SAGI_EXCEPT(SException::ERR_INVALID_STATE,
				"Invalid entity without vertex data.",
				"SEntity::locateToCenter");
		}
	...
	//  [1/5/2009 zhangxiang]
	SVertexData *SEntity::getVertexData(void) const{
		return m_pVertexData;
	}

	//  [1/5/2009 zhangxiang]
	SIndexData *SEntity::getIndexData(void) const{
		return m_pIndexData;
	}

	//  [8/1/2008 zhangxiang]
	//  [1/5/2009 zhangxiang]
	void SEntity::getVertexBuffer(SVertexData *outVBuffer, SVertexIndexBuffer *outIBuffer) const{
		if(!m_pVertexData || !m_pIndexData){
			THROW_SAGI_EXCEPT(SException::ERR_INVALID_STATE,
				"Invalid entity without vertex data.",
				"SEntity::getVertexBuffer");
		}
...
//		IndexedPolygon<m_iPolyType> *vIndexData = static_cast<IndexedPolygon<m_iPolyType>*>(vIndex->data());
		...
		// vertices
		Vector3 *outvBufferData = static_cast<Vector3*>(outVBuffer->createElement(SVertexBufferElement::ET_VERTEX, 3, outVertexNum)->data());
...
//		IndexedPolygon<m_iPolyType> *outIBufferData = static_cast<IndexedPolygon<m_iPolyType>*>(outIBuffer->data());
		...
		// colors
		SVertexBufferElement *cBuffer = m_pVertexData->getElement(SVertexBufferElement::ET_COLOR);
		if(cBuffer){
			Color *cBufferData = static_cast<Color*>(cBuffer->data());
			Color *outcBufferData = static_cast<Color*>(outVBuffer->createElement(SVertexBufferElement::ET_COLOR, 4, outVertexNum)->data());
			if(m_bSmooth){
		...
		// normals
		SVertexBufferElement *nBuffer = m_pVertexData->getElement(SVertexBufferElement::ET_NORMAL);
		if(m_pFaceNormalBuffer){
			Vector3 *outnBufferData = static_cast<Vector3*>(outVBuffer->createElement(SVertexBufferElement::ET_NORMAL, 3, outVertexNum)->data());
			if(m_bSmooth){
		...
		// texture coordinates
		SVertexBufferElement *tBuffer = m_pVertexData->getElement(SVertexBufferElement::ET_TEXTURE_COORD);
		SVertexIndexBuffer *tIndex = m_pIndexData->getElement(SVertexBufferElement::ET_TEXTURE_COORD);
		if(tBuffer){
			if(m_bSmooth){
				Vector2 *outtBufferData = static_cast<Vector2*>(outVBuffer->createElement(SVertexBufferElement::ET_TEXTURE_COORD, 2, outVertexNum)->data());
	...
	SBuffer *SEntity::getFaceNormalBuffer(void) const{
	...
	: SEntity(1, 1, 1){
		...
		m_RenderOption.setRenderElementType(SRenderOption::RET_POINTS);

		m_pPosition = static_cast<Vector3*>(m_pVertexData->createElement(SVertexBufferElement::ET_VERTEX, 3, m_iVertexNum)->data());
		*m_pPosition = aPos;

		m_pColor = static_cast<Color*>(m_pVertexData->createElement(SVertexBufferElement::ET_COLOR, 4, m_iVertexNum)->data());
		*m_pColor = aColor;
	...
	void SLine::init(const SVertex &aV0, const SVertex &aV1){
		...
		m_pPosData = static_cast<Vector3*>(m_pVertexData->createElement(SVertexBufferElement::ET_VERTEX, 3, m_iVertexNum)->data());
		m_pPosData[0] = aV0.position();
		m_pPosData[1] = aV1.position();

		m_pColorData = static_cast<Color*>(m_pVertexData->createElement(SVertexBufferElement::ET_COLOR, 4, m_iVertexNum)->data());
	...
	STriangle::STriangle(const SVertex &av0,
						...
						const SVertex &av2,
						bool aCounterClockWise/* = true*/)
	: SEntity(3, 3, 1, false, aCounterClockWise){
