//////////////////////////////////////////////////////
// file: SD3D9Renderer.cpp @ 2008-8-6 by Zhang Xiang
// defines of the class SD3D9Renderer
// SD3D9Renderer is a class ...
//////////////////////////////////////////////////////

// INCLUDES //////////////////////////////////////////
#include "SD3D9Renderer.h"
...
namespace Sagitta{
	...
	//  [8/7/2008 zhangxiang]
	SD3D9Renderer::SD3D9Renderer(HWND aHWnd, int aTWidth, int aTHeight) :
	SRenderer(aTWidth, aTHeight, true),
	...
	//  [8/7/2008 zhangxiang]
	SD3D9Renderer::~SD3D9Renderer(void){
		// shutdown();
		SAFE_RELEASE(m_pD3D);
	...
	//  [8/7/2008 zhangxiang]
	void SD3D9Renderer::init(void){
		if((m_pD3D = Direct3DCreate9(D3D_SDK_VERSION)) == NULL)
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Create D3D failed.", "SD3D9RenderSystem::SD3D9RenderSystem");

		D3DDISPLAYMODE  d3ddm;
		ZeroMemory(&d3ddm, sizeof(d3ddm));
		if(FAILED(m_pD3D->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &d3ddm)))
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Get D3D display mode failed.", "SD3D9RenderSystem::SD3D9RenderSystem");
		...
		if(FAILED(m_pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_HWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &d3dpp, &m_pDevice))){
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Create D3D device failed.", "SD3D9RenderSystem::SD3D9RenderSystem");
		}
	...
	void SD3D9Renderer::setViewport(SViewport *aViewport) const{

	}

	//  [1/15/2009 zhangxiang]
	void SD3D9Renderer::clearFrameBuffers(uInt aFlags, const Color &aBkColor,
		...
		//	StdString msg = DXGetErrorDescription9(hr);
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR,
				"Error clearing frame buffer : "/*  + msg*/, "D3D9Renderer::clearFrameBuffers" );
		}
	}

	//  [1/15/2009 zhangxiang]
	void SD3D9Renderer::setProjMatrix(const Matrix4 &aMatrix) const{
		// set projection matrix
		m_pDevice->SetTransform(D3DTS_PROJECTION, (D3DMATRIX*)(aMatrix.transpose().arr()));
	}

	//  [1/15/2009 zhangxiang]
	void SD3D9Renderer::setViewMatrix(const Matrix4 &aMatrix) const{
		// set view matrix
		m_pDevice->SetTransform(D3DTS_VIEW, (D3DMATRIX*)(aMatrix.transpose().arr()));
	}

	//  [1/10/2009 zhangxiang]
	int SD3D9Renderer::setupLightsImpl(const Color &aGlobalAmbiantColor) const{
		// for future ...
		return 0;
	}

	//  [1/15/2009 zhangxiang]
	void SD3D9Renderer::render(const SRenderOption &aGlobalRop, SRenderable *aRenderable) const{
		// setup material
		...
		for(; elemIt.hasMoreElements(); elemIt++, elemIndex++){
			...
			switch(element->type()){
					...
					default:
						{
							THROW_SAGI_EXCEPT(SException::ERR_INVALID_STATE, "Invalid render state.", "SD3D9RenderSystem::render");
							break;
						}
			}
			...
			if(FAILED(m_pDevice->CreateVertexBuffer(sizeof(element->data()), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, &pD3DVB[elemIndex], NULL)))
				THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Create D3D vertex buffer failed", "SD3D9RenderSystem::render");
			void *data = NULL;
		...
		if(FAILED(m_pDevice->CreateVertexDeclaration(d3delments, &m_pVertexDeclaration)))
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Create declaration failed.", "SD3D9RenderSystem::render");
		if(FAILED(m_pDevice->SetVertexDeclaration(m_pVertexDeclaration)))
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Set declaration failed.", "SD3D9RenderSystem::render");
		...
		if(FAILED(m_pDevice->CreateIndexBuffer(sizeof(pvib->data()), D3DUSAGE_WRITEONLY, D3DFMT_INDEX32, D3DPOOL_DEFAULT, &pD3DVIB, NULL)))
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Create D3D index buffer failed.", "SD3D9RenderSystem::render");
		void *pIndex = NULL;
		...
		if(FAILED(D3DXCreateMatrixStack(0, &pStack)))
			THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Create D3D matrix stack failed.", "SD3D9RenderSystem::render");
		...
		m_pDevice->BeginScene();
		{
			...
			for(uInt i=0; i<elemIndex; i++){
				if(FAILED(m_pDevice->SetStreamSource(d3delments[i].Stream, pD3DVB[i], d3delments[i].Offset, vertexsize[i])))
					THROW_SAGI_EXCEPT(SException::ERR_RENDERINGAPI_ERROR, "Set D3D stream source failed.", "SD3D9RenderSystem::render");
			}
			// set indeices
	...
	//  [1/15/2009 zhangxiang]
	void SD3D9Renderer::postRenderImpl(void) const{
		// render over
		m_pDevice->Present(0, 0, 0, 0);
	}

	//  [1/10/2009 zhangxiang]
	void SD3D9Renderer::resetLights(int aLightNum) const{
		// for future ...
	}

	//  [8/7/2008 zhangxiang]
	int SD3D9Renderer::retMapping(int aRet) const{
		switch(aRet){
