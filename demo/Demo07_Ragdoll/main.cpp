...
namespace opttest
{
    ...
    void mytestRos()
    {
        ...
        while(err > thr && count < 40000)
        {
            ...
            Matrix pmmjac = mmjact * mmjacplus;
    ...
    void meyer(const real_1d_array &x, real_1d_array &fi, void *ptr)
    {
    
        double ui;
        const real_1d_array &p = x;
        for(int i=0; i<x.length(); ++i){
            ui=0.45+0.05*i;
            fi[i]=p[0]*exp(10.0*p[1]/(ui+p[2]) - 13.0);
        }
    }

    void jacmeyer(const real_1d_array &x, real_1d_array &fi, real_2d_array &jac, void *ptr)
    {
        int i,j;
        double ui, tmp;
        const real_1d_array &p = x;
