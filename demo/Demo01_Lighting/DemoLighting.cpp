...
void DemoLighting::prepare(void)
{
	if(mScene && mCamera)
	{
		...
		// place sphere
		sgSceneObject *objSphere = (sgSceneObject*)sgObject::createObject(sgSceneObject::GetClassTypeName());
		objSphere->setParent(mScene->getRoot());
		objSphere->translate(Vector3(2.0f, 0.0f, -2.5f));
		objSphere->scale(Vector3(0.5f, 0.5f, 0.5f));
    
		sgMeshComponent *sphereMeshComp = (sgMeshComponent*)objSphere->createComponent(sgMeshComponent::GetClassTypeName());
		sphereMeshComp->setMeshFile(meshSphere->getFilename());
    
		// place cone
		sgSceneObject *objCone = (sgSceneObject*)sgObject::createObject(sgSceneObject::GetClassTypeName());
		objCone->setParent(mScene->getRoot());
		objCone->translate(Vector3(0.0f, -0.5f, -2.5f));
		objCone->scale(Vector3(0.5f, 2.0f, 0.5f));
    
		sgMeshComponent *coneMeshComp = (sgMeshComponent*)objCone->createComponent(sgMeshComponent::GetClassTypeName());
